...
`opcionempleo_scraper.py` también existe pero está **desactivado**: el sitio usa Cloudflare Turnstile y devuelve HTTP 200 con una página de "Verificación requerida" en vez del contenido real, incluso probando con Playwright headless y con `playwright-stealth` — esto hace que `verificar_link_activo()` nunca pueda confirmar ni descartar la vigencia real de sus ofertas (quedan como "no concluyente por verificación anti-bot"), lo que dejó pasar al menos una oferta ya vencida sin detectar en el dashboard. Reemplazado por `kitempleo_scraper.py` (HTML SSR clásico sin Cloudflare, filtrado por keyword server-side verificado con dos keywords distintas → IDs de empleo distintos) para mantener 10 fuentes activas.
